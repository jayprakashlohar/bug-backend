...
// //Find by id
// cartRouter.get("/:userId/:id", async (req, res) => {
//   const { userId, id } = req.params;
//   try {
//     const product = await CartModel.findOne({ userId, _id: id });
//     res.status(200).json(product);
//   } catch (err) {
//     res.status(500).send(err.message);
//   }
...
// //Add new product
// cartRouter.post("/:userId/createproduct", async (req, res) => {
//   const { userId } = req.params;
//   try {
//     const payload = { ...req.body, userId };
//     const { _id } = payload;

//     const product = await CartModel.findOne({ userId, _id });
...
// //Delete product
// cartRouter.delete("/:userId/delete/:id", async (req, res) => {
//   const { userId, id } = req.params;
//   try {
//     await CartModel.findOneAndDelete({ userId, _id: id });
//     res.send({ msg: "Product deleted successfully" });
//   } catch (err) {
//     res.send(401).send({ msg: "Something Went Wrong" });
//   }
...
// cartRouter.patch("/:userId/update/:id", async (req, res) => {
//   try {
//     const { userId, id } = req.params;
//     const { data } = req.body;
//     await CartModel.findOneAndUpdate({ userId, _id: id }, data);
//     res.status(200).send({ msg: "Data updeted", data: data });
//   } catch (error) {
//     console.log("err", error);
//     res.status(400).json({ message: error.message });
